...
3. Confirm the `genealogy-evidence` skill appears in Customize

Note: Grok Bot loads plugins from the Cursor dashboard/marketplace, not from `~/.cursor/plugins/local`. Local install still matters for Cursor IDE verification.

### From a Git repository

Push this folder to a public Git repo, then install via your client's plugin/git install flow or list it on [cursor.directory](https://cursor.directory). Do not submit to the Cursor Marketplace unless the owner explicitly asks.
